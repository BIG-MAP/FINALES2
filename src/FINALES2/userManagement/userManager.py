...
class UserDB:
    ...
    def getSingleUser(self, username: str) -> User:
        ...
        Outputs:
        singleU: an instance of the User class with the attributes set according to the
        ...
        if len(row) == 1:
            ...
            # If there are more than one or no user with this username,
            # raise an exception
            raise HTTPException(
                status_code=403,
    ...
    def getAllUsers(self) -> list[User]:
        ...
        Outputs:
        allUsers: a list of objects of the class User, each having its attributes set
...
def createUser(
    ...
    new_user = userDB_db.getSingleUser(username=new_user.username)
    userDB_db.closeConnection()
    return new_user
...
def singleUser(username: str, userDB: str = config.userDB) -> dict:
    ...
    Outputs:
    thisUser: a dictionary created based on the attributes of the user object collected
    ...
    thisUser_dict = thisUser.to_dict()
    return thisUser_dict
...
def allUsers(userDB: str = config.userDB) -> list[dict]:
    ...
    Outputs:
    allUsers: a list of dictionaries comprising a dictionary for each user collected
              from the user database
    """
    ...
    allUsers_dict = [user.to_dict() for user in allUsers]
    return allUsers_dict
...
def getActiveUser(
    ...
    Outputs:
    activeUser: a user object created based on a query to the user database with the
...
def getAccessToken(
    ...
    expirationMin: a timedelta giving the duration until the expiration of the token
                   in minutes or None
    ...
    # Save the expiration time to the tokenData using the key "exp" (it needs to be
